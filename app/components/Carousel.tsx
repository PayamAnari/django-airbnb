      ...
      <div className="overflow-hidden w-full" ref={carouselRef}>
        ...
        >
          {data.map((item, index) => (
            <div
              key={index}
              className={`flex flex-col items-center w-16 mx-8 cursor-pointer border-b-2 ${dataCategory === item.title ? ' border-black opacity-100' : 'border-white opacity-60 '}  hover:border-gray-300 hover:opacity-100 `}
            ...
            >
              ...
              />
              <span className={`text-xs mt-2 whitespace-nowrap ${dataCategory === item.title ? ' text-black ' : 'text-gray-500 '}`}  >{item.title}</span>
            </div>
          ))}
        </div>
      </div>
